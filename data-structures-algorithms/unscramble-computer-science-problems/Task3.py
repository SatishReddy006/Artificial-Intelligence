...
for call in calls:
    if call[0][0:5] == '(080)':
        if call[1][:2] == '(0':
            area_codes.append(call[1].split(sep=')')[0] + ')')
        elif call[1][:3] == '140':
            area_codes.append(call[1][:3])
        elif call[1][0] in ['7','8','9']:
            area_codes.append(call[1][:4])

area_codes_set = list(set(area_codes))
area_codes_set.sort()

print("The numbers called by people in Bangalore have codes:")
for area_code in area_codes_set:
    print(area_code)
...
for record in calls:
    if record[0][:5]=='(080)': 
        count1+=1
    if record[0][:5]=='(080)' and record[1][:5] =='(080)':
        count2+=1
